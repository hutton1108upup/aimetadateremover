...
const faqs = [
  { question: "Can I use ImageFinisher without an account?", answer: "Yes. The Guest plan is designed for one free single-image clean per day. A free Google account increases that allowance to three single-image cleans per day in total, not four. Metadata inspection is free on every plan." },
  { question: "What counts as a batch task?", answer: "One batch task processes one selected group of JPEG or PNG images with one set of cleaning options and prepares a ZIP download. The limit is 30 images and 200 MB on desktop, or 10 images and 100 MB on mobile. Each image must be 25 MB or smaller. Single-image cleans use your three free daily cleans first. After those are used, a single-image clean on Batch Pro uses one batch task." },
  { question: "When do daily allowances reset?", answer: "Daily allowances reset at 00:00 UTC each day (08:00 in China and Singapore). Unused daily allowances do not roll over. Your daily reset is separate from the paid plan’s calendar-month renewal cycle." },
  { question: "Is $4.99 the price per day?", answer: "No. Batch Pro costs USD 4.99 per calendar month, with up to 20 batch tasks per day while the subscription is active. It renews automatically each month unless canceled. For example, a September 23 purchase renews on October 23. Your checkout and account billing show the exact renewal date, including how shorter months are handled. Applicable taxes are shown before payment." },
  { question: "How will cancellation and refunds work?", answer: "You can cancel auto-renewal from Account billing before the next renewal. Canceling stops future charges; access continues until the end of the paid period. Our policy allows a full refund for the first purchase within seven days if no paid batch task has been used. Duplicate charges and service failures are reviewed separately, and mandatory consumer rights are preserved. See the Terms of Service for details." },
  ...
  { question: "Can I subscribe today?", answer: "Current availability is shown on the Batch Pro card below. When subscriptions are open, sign in with Google, review the monthly renewal terms and continue to secure checkout. A payment is confirmed by our server before paid access is activated. You can check your status and cancel future renewals in Account billing." },
];
const rows = [
  ...
  ["Automatic renewal", "None", "None", "$4.99 per calendar month"],
];
...
export default function PricingPage() {
  return <><PublicHeader /><main className="pricing-page">
    <section className="pricing-hero"><div className="page-container">
      ...
      <aside className="pricing-launch-note" aria-label="Plan availability"><Info size={20} aria-hidden="true" /><p><strong>Monthly subscription, daily allowances.</strong> Batch Pro renews each calendar month. Daily cleaning allowances reset separately at 00:00 UTC. Check the Batch Pro card for current purchase availability.</p></aside>
      <div className="pricing-plans">
        ...
        <PlanCard featured name="Batch Pro" audience="For creators preparing more images at once." price={`$${pricing.amountUsd}`} priceNote="USD / month" allowance={`${pricing.batchJobsPerDay} batch tasks / day`} allowanceNote="Up to 30 images per batch on desktop." features={["Everything in Free account", "Up to 10 images per batch on mobile", "Batch cleaning and individual verification", "Download cleaned files together as a ZIP"]} action={<><BillingPanel compact /><p className="plan-renewal">Renews automatically each calendar month. Cancel future renewals in Account billing.</p></>} />
      </div>
      <p className="pricing-fineprint">Daily allowances reset at 00:00 UTC. No daily rollover. Batch Pro is USD $4.99 per calendar month, plus applicable taxes shown at checkout. <Link href="/terms#fees">Billing & refund terms</Link>.</p>
