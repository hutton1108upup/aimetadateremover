...
export async function POST(request:Request) {
  try {
    ...
    if (await db.prepare("SELECT event_id FROM billing_event WHERE environment=? AND event_id=?").bind(config.environment,event.id).first()) return json({received:true});
    ...
    await db.prepare("INSERT OR IGNORE INTO billing_event(environment,event_id,event_type,order_id,received_at) VALUES(?,?,?,?,?)").bind(config.environment,event.id,event.eventType,event.data.orderId,Date.now()).run();
