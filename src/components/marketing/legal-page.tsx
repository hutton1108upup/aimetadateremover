...
export function LegalPage({ label, h1, lede, sections }: { label: string; h1: string; lede: string; sections: LegalSection[] }) {
  return <InfoPage label={label} h1={h1} lede={lede}>
    <article className="legal-document">
      <p className="legal-version">Version {legalDetails.version} · Effective date: <time dateTime={legalDetails.effectiveDate}>{legalDetails.effectiveDateLabel}</time> · Last updated: <time dateTime={legalDetails.effectiveDate}>{legalDetails.effectiveDateLabel}</time></p>
