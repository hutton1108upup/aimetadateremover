...
export function ToolPage(props: ToolPageProps) {
  return <><PublicHeader /><main><section className="tool-hero"><div className="page-container"><Breadcrumbs current={props.label} /><div className="tool-hero-copy"><p className="eyebrow">Free · runs locally · no account</p><h1>{props.h1}</h1><p>{props.intro}</p></div><UnifiedImageWorkspace variant="embedded" defaultMode={props.defaultMode} acceptedFormats={props.acceptedFormats} /></div></section>
    ...
    <section className="section section-alt"><div className="page-container"><div className="section-heading"><p className="eyebrow">How it works</p><h2>See what is there before you remove anything</h2></div><ol className="step-list">{props.steps.map((step,index)=><li key={step.title}><span>{index+1}</span><div><h3>{step.title}</h3><p>{step.body}</p></div></li>)}</ol></div></section>
    <FAQSection title={`Questions about ${props.label.toLowerCase()}`} items={props.faqs} />
    <section className="section section-alt"><div className="page-container related-card"><span><Eye /><b>What next?</b></span><div><Link href="/">AI metadata cleaner <ArrowRight /></Link>{props.path !== "/metadata-checker" && <Link href="/metadata-checker">Inspect before removing <ArrowRight /></Link>}<Link href="/workspace">Open the full workspace <ArrowRight /></Link></div></div></section>
