...
import { Check, ChevronDown, Download, FileImage, FolderOpen, LockKeyhole, ScanSearch, ShieldCheck, Sparkles, UploadCloud, X } from "lucide-react";
        ...
        <div className="tool-stage">
          ...
          {!active ? (
            <div className="dropzone" role="button" aria-label="Open file picker" onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); inputRef.current?.click(); } }} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); void addFiles(Array.from(event.dataTransfer.files)); }} onPaste={(event) => void addFiles(Array.from(event.clipboardData.files))} tabIndex={0}>
              ...
              <div className="drop-actions"><button className="button primary" onClick={() => inputRef.current?.click()} disabled={busy}><FolderOpen aria-hidden="true" /> Choose images</button><button className="button secondary" onClick={() => inputRef.current?.click()}><ScanSearch aria-hidden="true" /> Inspect first</button></div>
            ...
            <div className="active-workspace">
              <div className="preview-panel">
                <div className="preview-toolbar"><span>{active.file.name}</span><span className="preview-tools"><button className={previewView === "original" ? "selected" : ""} onClick={() => setPreviewView("original")} aria-label="Show original">Original</button><button className={previewView === "cleaned" ? "selected" : ""} onClick={() => setPreviewView("cleaned")} disabled={!active.cleanPreview} aria-label="Show cleaned">Cleaned</button><button onClick={() => setZoom((value) => Math.max(50, value - 25))} aria-label="Zoom out">−</button><b>{zoom}%</b><button onClick={() => setZoom((value) => Math.min(200, value + 25))} aria-label="Zoom in">+</button><button onClick={() => inputRef.current?.click()} aria-label="Add images">+</button><button onClick={() => removeFile(active.id)} aria-label={`Remove ${active.file.name}`}><X /></button></span></div>
                <div className="image-stage">{previewSrc ? <img style={{ transform: `scale(${zoom / 100})` }} src={previewSrc} alt={`${previewView === "cleaned" ? "Cleaned" : "Original"} file preview`} /> : <FileImage aria-hidden="true" />}</div>
              ...
              <div className="action-panel">
                <div className="tool-tabs" role="tablist" aria-label="Workspace steps">{(["inspect", "clean", "humanize", "export"] as ToolTab[]).map((item) => <button id={`tab-${item}`} aria-controls={`panel-${item}`} role="tab" aria-selected={tab === item} key={item} onClick={() => setTab(item)}>{item}</button>)}</div>
