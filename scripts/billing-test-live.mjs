...
const secretPath="artifacts/secrets/billing-test.json",fixturePath="artifacts/secrets/billing-test-fixture.json";
...
if(command==="configure"){
  const stored=spawnSync(process.execPath,["node_modules/wrangler/bin/wrangler.js","secret","bulk",secretPath,"--config","wrangler.billing-test.jsonc"],{encoding:"utf8",env:process.env});if(stored.status!==0)throw new Error("Test secret storage failed; inspect local Wrangler logs.");
  if(!fixture){const token=randomBytes(32).toString("hex"),id=randomUUID(),now=Date.now();await sql("INSERT INTO auth_user(id,name,email,email_verified,created_at,updated_at) VALUES(?,?,?,?,?,?)",[id,"ImageFinisher Test Buyer","billing-test@example.test",1,now,now]);await sql("INSERT INTO auth_session(id,token,user_id,expires_at,created_at,updated_at) VALUES(?,?,?,?,?,?)",[randomUUID(),token,id,now+86400000,now,now]);const cookie=(await serializeSignedCookie("__Secure-better-auth.session_token",token,secrets.AUTH_SECRET,{secure:true,path:"/",httpOnly:true})).split(";")[0];fixture={id,cookie};await writeFile(fixturePath,JSON.stringify(fixture),{mode:0o600});}
