...
更新日期：2026-09-24。本文记录实施状态，不把“写了代码”当成“已上线收款”。

## 2026-09-24 人工客服确认与现场核查（优先于下方历史记录）

用户提供的人工客服回信确认：

1. `support@aimetadataremover.pro` 已于 9 月 23 日 14:04 UTC 验证并绑定，无需再发送验证码或解绑邮箱。
2. 旧域名仍需 Waffo 人工释放，团队正在处理，会另行邮件通知。
3. 释放后优先使用已验证邮箱同域名的一键 `Verify domain`；只有该选项不可用时才改用 DNS TXT。
4. 更换域名不会关闭店铺生产资格；新域名验证成功后，网站字段自动更新，无需重新提交整份 KYB。可能的补充审核与店铺生产资格并行，不作为暂停生产的前置条件。这是本店铺人工支持给出的明确说明，修正下方此前依据通用文档提出的重新审核步骤。
5. 客服已检查新站首页、价格、条款和隐私页面，没有提出需要先修改这些页面的要求。

本轮实际生产 API 与 General 页面核查：客服邮箱确已变为新地址；网站仍为 `https://aimetadateremover.pro/`，页面仍显示“店铺已开启生产,网站已锁定”，尚无新域名验证入口。没有重复解绑邮箱、修改 DNS 或提交 KYB。

下一步：收到人工释放通知后，打开本店铺 General → 确认域名输入/验证入口已解锁 → 填写 `https://aimetadataremover.pro/` → 使用同域名邮箱的一键验证 → 同时核对页面“已验证”和生产 API 的网站字段。旧绑定尚未释放时无需用户重复发信。

注意：Waffo 店铺的 `prodEnabled=true` 只表示支付平台允许生产业务，不代表项目已经发布 Batch Pro 的正式收费功能。本项目的正式商品、部署和剩余验收仍按下方发布门槛推进。
